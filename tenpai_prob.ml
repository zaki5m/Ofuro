...
let tenpai_kitaiti_pp lst f_lst zi_kaze ba_kaze naki yaku_lst dora_lst =
  let (t_ary,t_zi_ary) = list_to_ary lst in
  let ten_lst = tehai_to_ten t_ary t_zi_ary zi_kaze ba_kaze naki f_lst yaku_lst dora_lst in
  let m = List.length ten_lst in
...
let tenpai_to_opt tehai tumo_l rm_wan f_lst zi_kaze ba_kaze naki yaku_lst dora_lst ary zi_ary = 
  let rec loop tmp double_lst i t_lst = match t_lst with 
    ...
    | h::t -> if List.exists (fun a -> a = h) double_lst then 
                ...
                let tmp = 
                  if n = 0 then
                    (i,(tenpai_kitaiti_pp lst f_lst zi_kaze ba_kaze naki yaku_lst dora_lst))::tmp
                  else
                    tmp
                in
...
let tenpai_to_opt_p tehai f_lst zi_kaze ba_kaze naki yaku_lst dora_lst = 
  let rec loop tmp double_lst i t_lst = match t_lst with 
    ...
    | h::t -> if List.exists (fun a -> a = h) double_lst then 
                ...
                let tmp = 
                  if n = 0 then
                    (tenpai_kitaiti_pp lst f_lst zi_kaze ba_kaze naki yaku_lst dora_lst)::tmp
                  else
                    tmp
                in
                let i = 
...
*)
(*
let opt_tumohai tenpai_lst = 
  let rec loop tmp t_lst = match t_lst with 
    | [] -> tmp 
    | (k_lst,tumo_lst,current_tehai)::t -> let tmp = (all_tumo (k_lst,tumo_lst,current_tehai))@tmp in
                                                         loop tmp t 
  in
  loop [] tenpai_lst
  *)
...
let make_agariritu_kitaiti rest_tumo_lst (new_tumo_lst:(int*float)list) tumo_len rm_wan = 
  let m = List.length rest_tumo_lst in 
  let len = 69 - (int_of_float rm_wan) in
  let rec loop (tmp,tmp2) t_lst = match t_lst with 
    | [] -> (tmp,tmp2) 
    | (h1,h2)::t -> let t_ritu = 
                if m = 0 then 
...
let tenpai_to_kitaiti ary zi_ary tenpai_lst f_lst zi_kaze ba_kaze naki dora_lst tumo_l rm_wan = 
  ...
  let rec loop tmp t_lst = match t_lst with 
    ...
    | (k_lst,tumo_lst,rest_tumo_lst,current_tehai,t_ritu)::t -> let (ary2,zi_ary2) = ary_opt ary zi_ary tumo_lst in
                                                                ...
                                                                let a_k_lst = tenpai_to_opt_p current_tehai f_lst zi_kaze ba_kaze naki [] dora_lst in
...
let table_json ary zi_ary =
  ("table", `List [`Int ary.(0).(0); `Int ary.(0).(1); `Int ary.(0).(2); `Int ary.(0).(3); `Int ary.(0).(4); `Int ary.(0).(5); `Int ary.(0).(6); `Int ary.(0).(7); `Int ary.(0).(8);
                   `Int ary.(1).(0); `Int ary.(1).(1); `Int ary.(1).(2); `Int ary.(1).(3); `Int ary.(1).(4); `Int ary.(1).(5); `Int ary.(1).(6); `Int ary.(1).(7); `Int ary.(1).(8);
                   `Int ary.(2).(0); `Int ary.(2).(1); `Int ary.(2).(2); `Int ary.(2).(3); `Int ary.(2).(4); `Int ary.(2).(5); `Int ary.(2).(6); `Int ary.(2).(7); `Int ary.(2).(8);
                   `Int zi_ary.(0); `Int zi_ary.(1); `Int zi_ary.(2); `Int zi_ary.(3); `Int zi_ary.(4); `Int zi_ary.(5); `Int zi_ary.(6)]) 

let dahaikouho hand f_lst ary zi_ary dora zi_kaze ba_kaze turn = 
  ...
  let table = table_json ary zi_ary in 
  `Assoc
    [
      dahai;
      furo_json;
...
let client_fun ic oc hand f_lst ary zi_ary dora zi_kaze ba_kaze turn = 
  ...
  let new_json = dahaikouho hand f_lst ary zi_ary dora zi_kaze ba_kaze turn in 
...
let prob_select sutehai_lst tehai furo_lst yaku_lst player yama_len zi_kaze ba_kaze naki dora_lst furo_double_lst =
  ...
  if mode_choice n tumo_l || n = n' && n' < 3 then 
    if List.exists (fun a -> a = Reach || a = Doublereach) yaku = true then
      ...
      let (dahai,_,_,_) = client_fun ic_kitaiti oc_kitaiti tehai (tapl_player furo_lst) ary zi_ary dora_lst zi_kaze ba_kaze (tumo_l+1) in 
      ...
      hai_to_int tehai x*)
      let (dahai,_,_,_) = client_fun ic_kitaiti oc_kitaiti tehai (tapl_player furo_lst) ary zi_ary dora_lst zi_kaze ba_kaze tumo_l in 
      hai_to_int tehai dahai
  else
...
let threthhold_furo_15 agariritu kitaiti tumo_len = 
  if tumo_len > 15 then 
    if (agariritu > 0. && (50000. *. agariritu *. agariritu -. 10000.) > kitaiti) then 
      true
    else
      false
  else if tumo_len > 12 then 
    ...
    if (agariritu > 0. && (79000. *. agariritu *. agariritu -. 6000.) > kitaiti) then 
      true
    else
      false
  else if tumo_len > 6 then 
    if (agariritu > 0. && (500. *. agariritu *. agariritu) > kitaiti) then 
      true
    else
      false
  else
    if (agariritu > 0. && (92900. *. agariritu *. agariritu) > kitaiti) then 
      true
    else
      false
...
let threthhold_furo_20 agariritu kitaiti tumo_len = 
  if tumo_len > 15 then 
    if (agariritu > 0. && (53600. *. agariritu *. agariritu -. 10000.) > kitaiti) then 
      true
    else
      false
  else if tumo_len > 12 then 
    if (agariritu > 0. && (55200. *. agariritu *. agariritu -. 10000.) > kitaiti) then 
      true
    else
      false
  else if tumo_len > 9 then 
    if (agariritu > 0. && (150000. *. agariritu *. agariritu -. 10000.) > kitaiti) then 
      true
    else
      false
  else if tumo_len > 6 then 
    if ((7882000.0 *. agariritu *. agariritu *. agariritu) > kitaiti && agariritu < 0.) || (agariritu > 0. && (0. *. agariritu *. agariritu) > kitaiti) then 
      true
    else
      false
  else
    if ((9992000.0 *. agariritu *. agariritu *. agariritu) > kitaiti && agariritu < 0.) || (agariritu > 0. && (300. *. agariritu *. agariritu) > kitaiti) then 
      true
    else
      false
...
let threthhold_furo_25 agariritu kitaiti tumo_len = 
  if tumo_len > 15 then 
    if (agariritu > 0. && (60000. *. agariritu *. agariritu -. 10000.) > kitaiti) then 
      true
    else
      false
  else if tumo_len > 12 then 
    if (agariritu > 0. && (9400. *. agariritu *. agariritu -. 1600.) > kitaiti) then 
      true
    else
      false
  else if tumo_len > 9 then 
    if (agariritu > 0. && (16300. *. agariritu *. agariritu -. 1000.) > kitaiti) then 
      true
    else
      false
  else if tumo_len > 6 then 
    if ((1012000.0 *. agariritu *. agariritu *. agariritu) > kitaiti && agariritu < 0.) || (agariritu > 0. && (500. *. agariritu *. agariritu) > kitaiti) then 
      true
    else
      false
  else
    if ((3242000.0 *. agariritu *. agariritu *. agariritu) > kitaiti && agariritu < 0.) || (agariritu > 0. && (1000. *. agariritu *. agariritu) > kitaiti) then 
      true
    else
      false
...
let patern_to_kitaiti tehai (x,y) f_lst p_f_lst ary zi_ary dora zi_kaze ba_kaze turn = 
  let rec loop (tmp,t_ritu,agariritu,kitaiti) t_lst = match t_lst with 
    | [] -> (t_ritu,agariritu,kitaiti)
    | h::t -> let (dahai,tmp_a,tmp_b,tmp_c) = client_fun ic_kitaiti oc_kitaiti tehai (h::f_lst) ary zi_ary dora zi_kaze ba_kaze turn in 
              let kuikae_lst = kuikae (x,y) h in 
              let dahai = convert_int_to_hai dahai in 
              if List.exists (fun a -> a = dahai) kuikae_lst then 
                loop (tmp,t_ritu,agariritu,kitaiti) t 
              else if tmp_c > kitaiti then 
                loop (h,tmp_a,tmp_b,tmp_c) t 
              else
                loop (tmp,t_ritu,agariritu,kitaiti) t 
  in
  let (furo_mentsu,t_ritu,agariritu,kitaiti) = loop (0.0,0.0,0.0) p_f_lst in 
  let t_ritu = if t_ritu = 1.1 then 1.0 else t_ritu in
  (furo_mentsu,t_ritu,a_ritu,kitaiti)
...
let purob_furo sutehai_lst tehai furo_lst yaku_lst player yama_len zi_kaze ba_kaze naki dora_lst (x,y) furo_double_lst furoritu_lst =
  ...
  if n = 0 || tumo_len = 0 || n = n' then
    ...
    let (ary,zi_ary) = furo_lst_to_rm_ary furo_lst furo_double_lst ary zi_ary in
    let p_f_lst = possible_furo_patern tehai (x,y) in
    let (_,f_t_ritu,f_agariritu,f_kitaiti) = patern_to_kitaiti tehai (tapl_player furo_lst player) p_f_lst ary zi_ary dora zi_kaze ba_kaze tumo_len in 
    ...
    if Array.length tenpai_lst = 0 || Array.length tenpai_lst = 1 && List.length tenpai_lst.(0) = 0 then 
      ...
      if f_kitaiti_lst = [] then 
        ...
        let (k_lst,t_ritu,agariritu,kitaiti,anzendo,_,_) = not_naki in
        *)
        let (dahai,t_ritu,agariritu,kitaiti) = client_fun ic_kitaiti oc_kitaiti tehai (tapl_player furo_lst) ary zi_ary dora_lst zi_kaze ba_kaze tumo_len in 
        let _ = 
  ...
  if n = 0 || tumo_len = 0 || n = n' then
    ...
    if Array.length tenpai_lst = 0 || Array.length tenpai_lst = 1 && List.length tenpai_lst.(0) = 0 then 
      ...
      if f_kitaiti_lst = [] then 
        ...
        *)
        (*if naki = false then *)
          if (f_agariritu -. agariritu) > 0.0 && furoritu_to_furo furoritu (f_agariritu -. agariritu) (kitaiti -.f_kitaiti) tumo_len && k_hai <> (1,Not_hai) then
            [(k_hai,f_hai)]
          else
            (*let ((k_hai,den),f_hai) = keiten tehai sutehai_lst (List.nth furo_lst player) p_f_lst yama_len (x,y) yaku_lst ary zi_ary in 
            if k_hai = (1,Not_hai) || den > 5 then 
