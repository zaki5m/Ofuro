...
let kiriban tehai_lst sutehai_lst ary_lst (x,y) player f_lst naki (yaku_lst:Mahjong_base.yaku list list) dora_lst kyoku ba kyotaku player_score honba = 
  ...
  let rec loop' tehai sutehai =
    ...
    let n = 
      (*if player = 0 then
        ...
        hai_eff_select sutehai_lst tehai (List.nth yaku_lst player) player
    in
    Printf.printf "%d\n" n; flush stdout;
...
let furo_inq2 lst ary_lst (x,y) = 
  let rec loop' i tmp = 
    let (m,n) = List.nth lst i in
    let tmp =
      if n <> [] then
        (*print_player m*)
        let len = what_furo n in
        if len <> [] then
          let (ary,_) = List.nth ary_lst m in
...
let furo_inq ary_lst furo_lst naki_lst (x,y) player yama_lst tehai_lst yaku_lst dora_lst ba kyoku kyotaku honba = 
  let (x,y) = hai_to_ary (x,y) in
  let rec loop' i tmp = 
  ...
  let rec loop2' i tmp = 
    ...
    let c_f_lst =
      if player = 3 then  
        ...
        loop2' (player+1) []
    in
    let c_f_lst2 = furo_inq2 c_f_lst ary_lst (x,y) in(*一つのfuro_hai*)
    if c_f_lst2 = [] then
...
let rec furo_loop yama_lst tehai_lst sutehai_lst ary_lst (x,y) player furo_lst naki_lst yaku_lst dora_lst kyoku ba honba kyotaku player_score furiten_lst =
  ...
  let (yama_lst',naki_lst',player',dora_lst,furo_lst',tehai_lst,sutehai_lst,ary_lst,yaku_lst,player_score,kyotaku,(x,y),(a,b,c,d)) =
    if n = true then
      ...
      let (tehai_lst,sutehai_lst,ary_lst,yaku_lst,player_score,kyotaku) = kiriban tehai_lst sutehai_lst ary_lst (x,y) player furo_lst (List.nth naki_lst player) yaku_lst dora_lst kyoku ba kyotaku player_score honba in 
      ...
      if (a,b,c,d) <> (0,0,0,0) then
        (yama_lst,naki_lst,player,dora_lst,furo_lst,tehai_lst,sutehai_lst,ary_lst,yaku_lst,player_score,kyotaku,(x,y),(a,b,c,d))
          ...
          let (furo_lst',naki_lst',player',yama_lst,dora_lst,tehai_lst,(a,b,c,d)) =
            furo_inq ary_lst furo_lst naki_lst (last_sutehai sutehai_lst player) player yama_lst tehai_lst yaku_lst dora_lst ba kyoku kyotaku honba in
          if (a,b,c,d) <> (0,0,0,0) then
            (yama_lst,naki_lst',player',dora_lst,furo_lst',tehai_lst,sutehai_lst,ary_lst,yaku_lst,player_score,kyotaku,(x,y),(a,b,c,d))
          else
            (yama_lst,naki_lst',player',dora_lst,furo_lst',tehai_lst,sutehai_lst,ary_lst,yaku_lst,player_score,kyotaku,(x,y),(a,b,c,d))
